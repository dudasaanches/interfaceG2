import customtkinter as tk

tk.set_appearance_mode("Dark")

janela = tk.CTk()
janela.title("Janela")
janela.geometry("400x350")
janela.configure(fg_color="#94d2bd")
janela.resizable(width=False, height=False)

colunas = list(range(13))
linhas = list(range(13))
janela.grid_columnconfigure(colunas,weight=1)
janela.grid_rowconfigure(linhas,weight=1)


texto1 = tk.CTkLabel(janela, text="",text_color="Black", font=("Arial",16),justify="center")
texto1.grid(row=4,column=6)

nomes = []
precos = []



def clique():
    nome = caixa1.get()
    nomes.append(nome)
    preco = int(caixa2.get())
    precos.append(preco)
    sum(precos)


def mostrar():
    valor = max(precos)
    valor = precos.index(valor)
    texto1.configure(text=f"O preco total é de {precos}, e o produto mais caro é {valor}")


btn1 = tk.CTkButton(janela, text="Adicionar", command=clique, width=50, height=30,fg_color="Black")
btn1.grid(row=7,column=6)

btn2 = tk.CTkButton(janela, text="Mostrar", command=mostrar, width=50, height=30,fg_color="Black")
btn2.grid(row=7,column=7)

caixa1 = tk.CTkEntry(janela, placeholder_text="Digite a primeira nota", width=200, height=30)
caixa1.grid(row=5, column=6)

caixa2 = tk.CTkEntry(janela, placeholder_text="Digite a segunda nota", width=200, height=30)
caixa2.grid(row=6, column=6)


janela.mainloop()